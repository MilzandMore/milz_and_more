class MandalaRund {
  ...
  init(container) {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      this.colorIndicators[i] = createDiv("").parent(sRow)
        .style('width', isMobile ? '10px' : '12px').style('height', isMobile ? '10px' : '12px')
        .style('border-radius', '50%').style('border', '1px solid rgba(255,255,255,0.2)');
  ...
  render() {
    ...
    let currentColors = this.getColorMatrix(this.colorSeed); // Die rotierte Matrix
    
    // Farbpunkte im UI aktualisieren (Sie rotieren jetzt mit!)
    for (let i = 1; i <= 9; i++) { 
      if(this.colorIndicators[i]) {
        this.colorIndicators[i].style('background-color', currentColors[i-1]); 
      }
    }
  ...
  buildSector() {
    ...
    let c11 = (c, t) => t.forEach(([r, k]) => m[r][k
