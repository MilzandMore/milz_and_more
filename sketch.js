...
function preload() {
    // Logo-Laden ist optional, damit kein 404 alles stoppt
...
function setup() {
    ...
    for (var i = 1; i <= 9; i++) {
        var sRow = createDiv("").parent(sliderPanel).style('display','flex').style('align-items','center').style('gap','8px').style('margin-bottom','2px');
        var dot = createDiv("").parent(sRow).style('width', '10px').style('height', '10px').style('border-radius', '50%');
...
function updateLayout() {
    ...
    var isMobile = windowWidth < 600;
    if (isMobile) {
        sliderPanel.style('top', 'auto').style('bottom', '0').style('left', '0').style('width', '100%').style('display', 'grid').style('grid-template-columns', 'repeat(3, 1fr)').style('padding', '10px 5px');
    } else {
    ...
    for (var i = 1; i <= 9; i++) {
        if (sliders[i]) sliders[i].style('width', isMobile ? '85px' : '100px');
    }
}

function draw() {
    background(255);
    if (!designSelect || !sliders[1]) return; // Schutz vor "undefined" Absturz

    var isMobile = windowWidth < 600;
    var design = designSelect.value();
    if (design === "Rund") sektGroup.show(); else sektGroup.hide();
    
    var rawVal = inputField.value() || "1";
    ...
    push();
    // Besseres Zentrieren: Platz für Slider unten lassen
    var yOff = isMobile ? -60 : 40;
    translate(width / 2, height / 2 + yOff);
    
    if (design === "Quadrat") {
        scale((min(width, height) / 850) * (isMobile ? 0.7 : 0.9));
        calcQuadratMatrix(drawCode);
        renderQuadrat(startDigit);
    } else if (design === "Rund") {
        scale((min(width, height) / 900) * (isMobile ? 0.75 : 0.95));
        renderRund(drawCode, startDigit);
    } else if (design === "Wabe") {
        scale((min(width, height) / 520) * (isMobile ? 0.4 : 0.48));
        renderWabe(drawCode, startDigit);
    }
    pop();

    if (logoImg) {
        var lW = isMobile ? 60 : 150;
        var lH = (logoImg.height / logoImg.width) * lW;
        image(logoImg, 20, isMobile ? height - 160 : height - lH - 20, lW, lH);
    }
}
...
function renderQuadrat(startDigit, target) {
    ...
    var ts = 16; ctx.stroke(0, 30); ctx.strokeWeight(0.5);
    for (var r = 0; r < 20; r++) {
        for (var c = 0; c < 20; c++) {
            var val = qMatrix[r][c];
            if (val > 0) {
...
function renderWabe(code, startDigit, target) {
    var ctx = target || window;
    var sz = 16.2; ctx.stroke(0, 30); ctx.strokeWeight(0.5);
...
function exportHighRes() {
    ...
    pg.pop();
    if (logoImg) {
        var lW = 600; var lH = (logoImg.height / logoImg.width) * lW;
