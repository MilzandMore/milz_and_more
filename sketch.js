...
function renderRund(code, startDigit, target) {
    ...
    var sc = (sektS) ? int(sektS.value()) : 8;
...
function renderWabe(code, startDigit, target) {
    ...
    var path = (dirSelect && dirSelect.value() === 'Innen') ? [...code, ...[...code].reverse()] : [...[...code].reverse(), ...code];
    for (var s = 0; s < 6; s++) {
...
function preload() {
    // Sichereres Laden: Fehler verhindern den Start nicht
...
function draw() {
    background(255);
    if (!designSelect || !inputField || !sliders[1]) return;
    var isMobile = windowWidth < 600;
