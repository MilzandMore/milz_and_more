class MandalaRund {
  ...
  init(container) {
    ...
    ["6","8","10","12","13"].forEach(s => this.sektS.option(s)); this.sektS.selected("8");
    ...
    this.updateLayout();
    [this.modeSelect, this.inputField, this.sektS, this.richtungS].forEach(e => e.input(() => redraw()));
  }
  ...
  render() {
    ...
    for (let i = 1; i <= 9; i++) this.colorIndicators[i].style('background-color', currentColors[i-1]);
  ...
  drawSector(m, colors) {
    let step = 20;
    let angle = TWO_PI / int(this.sektS.value());
  ...
  buildSector() {
    ...
    c11(4, [[11, 7], [8, 4]]); c11(5, [[10, 4], [10, 6], [9, 4
