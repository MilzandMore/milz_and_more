// 1. GLOBALE VARIABLEN
var inputField, modeSelect, designSelect, dirSelect, sektS, sliders = [], colorIndicators = [], sliderPanel, sektGroup;
var qMatrix = [];
var logoImg;
var codeDisplay; 
var isAdmin = false;
...
function setup() {
  ...
  function createUIGroup(labelTxt, element, wMobile, wDesktop) {
    ...
    if (element) {
        element.parent(group).style('width', isMobile ? wMobile : wDesktop).style('font-size', isMobile ? '11px' : '13px');
    }
    return group;
  }

  designSelect = createSelect(); designSelect.option('Mandala'); designSelect.option('Quadrat');
  createUIGroup("DESIGN", designSelect, "75px", "110px");
  ...
  codeDisplay = createSpan("").parent(codeGroup).style('color', '#ffffff').style('font-weight', '600');
  ...
  var saveBtn = createButton('DOWNLOAD').parent(topBar).style('margin-left', 'auto').mousePressed(exportHighRes);
...
function updateLayout() {
  var isMobile = windowWidth < 600;
  if (isMobile) {
    sliderPanel.style('bottom', '0').style('left', '0').style('width', '100%').style('display', 'grid').style('grid-template-columns', 'repeat(3, 1fr)').style('padding', '8px 4px');
...
// --- DEINE ORIGINAL MANDALA LOGIK (RUND) ---
function drawOriginalMandala(code, startDigit) {
  ...
  for (var s = 0; s < sc; s++) {
    push();
    rotate(s * TWO_PI / sc);
    var currentLevel = [...code];
    for (var r = 0; r < 8; r++) {
      for (var i = 0; i < currentLevel.length; i++) {
        var val = currentLevel[i];
        ...
        fill(hue(col), map(sVal, 20, 100, 15, saturation(col)), map(sVal, 20, 100, 98, brightness(col)));
...
function drawQuadrat(startDigit) {
  var ts = 16; stroke(0, 35); strokeWeight(0.5);
  for (var r = 0; r < 20; r++) {
    for (var c = 0; c < 20; c++) {
      var val = qMatrix[r][c];
      if (val !== 0) {
...
function getCodeFromText() { 
  ...
  var firstRow = []; for (var char of textStr) { if (charMap[char]) firstRow.push(charMap[char]); }
  var currentRow = firstRow; while(currentRow.length < 8) currentRow.push(9);
  while (currentRow.length > 8) { var nextRow = []; for (var i = 0; i < currentRow.length - 1; i++) { nextRow.push(ex(currentRow[i], currentRow[i+1])); } currentRow = nextRow; }
  return currentRow;
}
...
function exportHighRes() {
  ...
  var startDigit = baseCode[0] || 1;
  var drawCode = (dirSelect.value().includes('Innen')) ? [...baseCode].reverse() : baseCode;
  pg.push(); pg.translate(pg.width/2, pg.height*0.4); pg.scale(3.8);
  if (designSelect.value() === "Quadrat") { calcQuadratMatrix(drawCode); drawQuadrat_Export(startDigit, pg); } 
  else { drawMandala_Export(drawCode, startDigit, pg); }
  pg.pop();
  save(pg, 'Milz&More_Export.png');
}

// Export Helper (identisch zu draw, nur auf pg)
function drawQuadrat_Export(startDigit, pg) { /* ...wie drawQuadrat... */ }
function drawMandala_Export(code, startDigit, pg) { /* ...wie drawOriginalMandala... */ }
