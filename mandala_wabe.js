class MandalaWabe {
  ...
  ex(a, b) { return (a + b === 0) ? 0 : ((a + b) % 9 === 0 ? 9 : (a + b) % 9); }
  ...
  init(container) {
    ...
    this.modeS.option('Geburtstag'); this.modeS.option('Affirmation');
    ...
    this.dirS.option('Außen', 'nach außen'); this.dirS.option('Innen', 'nach innen');
    ...
    this.updateLayout();
    [this.inputD, this.dirS, this.modeS].forEach(e => e.input(() => redraw()));
  }
  ...
  updateLayout() {
    ...
    if (isMobile) {
      ...
      this.sliderPanel.style('top', '90px').style('left', '0').style('width', 'auto').style('display', 'flex').style('flex-direction', 'column').style('padding', '12px');
  ...
  render() {
    ...
    for (let i = 1; i <= 9; i++) {
      this.colorIndicators[i].style('background-color', this.colorMatrix[cKey][i - 1]);
    }
      ...
      for (let r = 1; r <= 16; r++) {
        for (let i = 0; i < r; i++) {
          ...
          if (val >= 1 && val <= 9) {
            ...
            fill(hue(col), map(this.sliders[val].value(), 20, 100, 15, saturation(col)), map(this.sliders[val].value(), 20, 100, 98, brightness(col)));
          } else fill(255);
