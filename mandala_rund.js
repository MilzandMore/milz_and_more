...
function setupRund() {
  ...
  // Die blaue Leiste ist jetzt IMMER ganz oben (top: 0)
  ...
  // Platz links lassen für die neuen Icons (ca. 60px)
  ...
  createSpan("SEKTOR").parent(sektorGroup).style('color','#bdc3c7').style('font-size','10px').style('font-weight','bold');
  ...
  let saveBtn = createButton('DOWNLOAD').parent(topBar).addClass('app-ui')
    .style('margin-left', 'auto').style('padding', '8px 15px').style('background','#fff').style('border','none').style('border-radius','5px').style('font-weight','bold');
  saveBtn.mousePressed(() => save('Mandala_Rund.png'));

  // Slider Panel - Deine 75px Vorgabe [cite: 2026-02-11]
...
function drawRund() {
  ...
  push();
  // Zentrierung korrigiert
  translate(width/2, isMobile ? height/2 : height/2 + 40);
  
  // Skalierung massiv erhöht, damit Rund = Wabe
  ...
  for(let i=0; i<sektoren; i++) {
    push();
    rotate(TWO_PI/sektoren * i);
    drawSektorRund(code, sektoren);
    pop();
  }
  pop();
}

function drawSektorRund(code, sektoren) {
  let rBase = 12;
  for(let r=0; r<code.length; r++) {
    ...
    noStroke();
    // Der Arc füllt jetzt den Sektor exakt aus
    arc(0, 0, (r+1)*rBase*2, (r+1)*rBase*2, 0, TWO_PI/sektoren + 0.01);
  }
}
