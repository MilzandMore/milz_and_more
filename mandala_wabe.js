...
function setupWabe() {
  var isMobile = windowWidth < 600; slidersWabe = [];
  let topBar = createDiv("").addClass('app-ui').style('position', 'fixed').style('top', '0').style('left', '0').style('width', '100%').style('background', '#2c3e50').style('display', 'flex').style('align-items', 'center').style('padding', '5px 10px').style('z-index', '5000').style('height', '65px');
  ...
  dirSelectWabe = createSelect().parent(dirGroup); dirSelectWabe.option('Außen'); dirSelectWabe.option('Innen');
  ...
  saveBtn.mousePressed(() => save('Wabe.png'));
  ...
  for (let i = 1; i <= 9; i++) {
    ...
    slidersWabe[i] = createSlider(20, 100, 85).parent(sRow).style('width', '75px'); // [cite: 2026-02-11]
...
function drawWabe() {
  let isMobile = windowWidth < 600; push();
  translate(width/2, isMobile ? height/2 - 20 : height/2 + 40); scale(isMobile ? 0.8 : 1.15);
  let code = inputFieldWabe.value().replace(/[^0-9]/g, "").split('').map(Number);
  calcWabeMatrixLogic(code);
  for (let i = 0; i < 6; i++) { push(); rotate(i * PI / 3); drawWabeSegment(); pop(); }
  pop();
}

function drawWabeSegment() {
  let size = 12; let h = sqrt(3) * size;
  for (let r = 0; r < 20; r++) { for (let c = 0; c <= r; c++) {
    let v = wMatrixWabe[r][c]; if (v > 0) {
      let sVal = slidersWabe[v].value(); let col = color(mapZWabe[v]);
      fill(hue(col), saturation(col) * (sVal/100), brightness(col)); stroke(0, 40);
      drawHex((c - r / 2) * size * 1.5, r * h * 0.75, size);
    }
  }}
}

function drawHex(x, y, s) { beginShape(); for (let a = 0; a < TWO_PI; a += PI / 3) { vertex(x + cos(a) * s, y + sin(a) * s); } endShape(CLOSE); }
...
function calcWabeMatrixLogic(code) {
  ...
  if (dir === 'Außen') {
    ...
    for (let r = 18; r >= 0; r--) { for (let c = 0; c <= r; c++) {
      let res = (wMatrixWabe[r+1][c] + wMatrixWabe[r+1][c+1]) % 9; wMatrixWabe[r][c] = (res === 0) ? 9 : res;
    }}
  } else {
    for (let i = 0; i < code.length; i++) wMatrixWabe[i][0] = code[i];
    for (let c = 1; c < 20; c++) { for (let r = c; r < 20; r++) {
      let res = (wMatrixWabe[r-1][c-1] + wMatrixWabe[r][c-1]) % 9; wMatrixWabe[r][c] = (res === 0) ? 9 : res;
    }}
  }
}
